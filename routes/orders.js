...
router.get('/', isAuthenticated, (req, res) => {
  const orders = prepare("SELECT o.*, e.title as event_title, e.date as event_date FROM orders o JOIN events e ON o.event_id = e.id WHERE o.user_id = ? ORDER BY o.created_at DESC").all(req.sessionId);
...
router.post('/create/:eventId', isAuthenticated, isActive, (req, res) => {
  ...
  const result = prepare('INSERT INTO orders (user_id, event_id, quantity, total_price, status) VALUES (?, ?, ?, ?, ?)').run(req.sessionId, event.id, quantity, totalPrice, 'draft_pesanan');
  res.redirect('/orders/' + result.lastInsertRowid);
});

router.get('/:id', isAuthenticated, (req, res) => {
  const isAdmin = req.sessionRole === 'admin';
  const order = prepare("SELECT o.*, e.title as event_title, e.date as event_date, e.location, e.poster_url FROM orders o JOIN events e ON o.event_id = e.id WHERE o.id = ? AND (o.user_id = ? OR ?)").get(req.params.id, req.sessionId, isAdmin ? 1 : 0);
...
router.post('/:id/checkout', isAuthenticated, (req, res) => {
  const order = prepare('SELECT * FROM orders WHERE id = ? AND user_id = ? AND status = ?').get(req.params.id, req.sessionId, 'draft_pesanan');
...
router.post('/:id/cancel', isAuthenticated, (req, res) => {
  const order = prepare("SELECT * FROM orders WHERE id = ? AND user_id = ? AND status IN ('draft_pesanan','menunggu_pembayaran')").get(req.params.id, req.sessionId);
...
router.post('/:id/refund', isAuthenticated, (req, res) => {
  const order = prepare("SELECT * FROM orders WHERE id = ? AND user_id = ? AND status = 'terkonfirmasi'").get(req.params.id, req.sessionId);
